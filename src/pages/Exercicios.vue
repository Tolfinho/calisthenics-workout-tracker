    ...
    <div class="container">
      <div class="page-header">
          <div class="page-title">
            <h1>Exercicios</h1>
            <h3 class="subtitle">Gerencie seus exercícios da calistenia</h3>
          </div>
          <button class="btn btn-primary" @click="showForm=!showForm">+ Novo exercício</button>
      </div>

      <div class="form-container" v-if="showForm">
        <h3 class="form-title">Novo exercício</h3>

        <label for="nome" class="label">Nome: </label>
        <input type="text" class="input mb-1" id="nome" v-model="nome" placeholder="Informe o nome do exercício">

        <label class="label">Categoria: </label>
        <select class="input mb-1" v-model="categoria">
        ...
        <textarea class="input mb-1" id="descricao" v-model="descricao" placeholder="Informe a descrição do exercício"></textarea>

        <div class="form-buttons">
          <button class="btn btn-outline-primary" @click="showForm=!showForm">Cancelar</button>
      ...
      <div class="exercicios-list-container">
        <div class="exercicios-list">
          <div class="exercicio" v-for="e in exercicios" :key="e.id">
            <p><b>Nome: </b>{{ e.nome }}</p>
            <p><b>Descrição: </b>{{ e.descricao }}</p>
            <hr>
            <button class="btn btn-danger" @click="apagar(e.id)">Apagar</button>
          </div>
        </div>
      </div>
    </div>
